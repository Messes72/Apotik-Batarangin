<script lang="ts">
	...
	interface IProps {
		...
		actions?: Snippet<[{ body: any; closeDropdown: () => void }]>;
	}
	...
	const closeDropdown = () => (activeDropdown = null);
			...
			<div class="grid h-full grid-cols-[20%_60%_20%] gap-0">
				...
				<div class="flex w-full items-center justify-start">
					{#if body.link_gambar_obat}
						<img
							src={body.link_gambar_obat.startsWith('http')
								? body.link_gambar_obat
								: body.link_gambar_obat.startsWith('/')
									? `${API_BASE_URL}${body.link_gambar_obat}`
									: `${API_BASE_URL}/${body.link_gambar_obat}`}
							alt={body.nama_obat || 'Product Image'}
							class="h-[59px] w-[59px] rounded-md object-cover"
					...
					{#if activeDropdown === index}
						<div class="relative">
							...
							>
								<div class="flex justify-end">
									{#if actions}
										{@render actions({ body, closeDropdown })}
									{/if}
								</div>
							</div>
						</div>
