<script lang="ts">
	...
	const dispatch = createEventDispatcher<{
		delete_success: void;
		delete_error: DeleteErrorDetail;
		closed: void;
	}>();
	let isLoading = false;
	let errorMessage = '';
					...
					use:enhance={() => {
						...
						return async ({ result }) => {
							...
							if (result.type === 'success') {
								...
								dispatch('delete_error', { message: errorMsg });
							} else {
								isOpen = false;
							}
						};
					}}
					id="deleteObatRacikForm"
				>
					<input type="hidden" name="id_obat_racik" value={id_obat_racik} />
