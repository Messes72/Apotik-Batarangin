...
export const load: PageServerLoad = async ({ fetch, url, locals }) => {
	...
	const apiUrl = `${env.BASE_URL3}/retur/20?page=${page}&page_size=${page_size}`;
	const stokUrl = `${env.BASE_URL3}/stokopname/stok/20`;
...
export const actions: Actions = {
	createReturnBarang: async ({ request, fetch, locals }) => {
		try {
			...
			const payload = {
				id_depo: "20", // nilai default sesuai permintaan
