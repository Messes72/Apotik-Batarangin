...
<div class="grid grid-cols-3 gap-6">
	{#each card_data as body, index}
		<div class="font-montserrat w-auto flex-shrink-0 items-center rounded-xl border border-[#D9D9D9] bg-white px-4 py-2 text-[20px]">
			<div class="grid h-full grid-cols-[20%_60%_20%] gap-0">
				<!-- Grid pertama -->
				<div class="flex w-full items-center justify-start">
					{#if body.link_gambar_obat}
						<img 
							src={body.link_gambar_obat.startsWith('http') ? body.link_gambar_obat : 
							     body.link_gambar_obat.startsWith('/') ? `${API_BASE_URL}${body.link_gambar_obat}` : 
							     `${API_BASE_URL}/${body.link_gambar_obat}`}
							alt={body.nama_obat || 'Product Image'} 
							class="w-[59px] h-[59px] object-cover rounded-md"
