<script lang="ts">
	...
	export let isKonfirmTerimaReturnOpen = false;
</script>
	...
	>
		<div class="my-auto {width} rounded-xl bg-white drop-shadow-lg" on:click|stopPropagation>
			<div class="flex items-center justify-between rounded-t-xl bg-[#6988DC] p-8">
				<div class="font-montserrat text-[26px] text-white">Catatan Return Barang</div>
				<button
			...
			<form class="mb-4 mt-6 px-8 pb-3">
				<div class="mt-2 flex flex-col gap-2">
					<TextArea id="alasan_terima" label="Apakah ada Catatan untuk Return Barang ini?" placeholder="Catatan" />
				</div>
				<div class="mt-2 flex justify-end">
					<button
						...
						on:click={() => {
							isOpen = false;
							isKonfirmTerimaReturnOpen = true;
						}}>KONFIRMASI</button
					>
				</div>
			</form>
