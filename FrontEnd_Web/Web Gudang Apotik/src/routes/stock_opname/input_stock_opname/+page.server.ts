...
export const load: PageServerLoad = async ({ fetch, url, locals }) => {
	...
	const apiUrl = `${env.BASE_URL3}/stokopname/stok/10?page=${page}&pagesize=${page_size}${keyword ? `&keyword=${keyword}` : ''}`;
...
export const actions: Actions = {
	getBatchDetails: async ({ request, locals, fetch }) => {
		...
		try {
			const batchApiUrl = `${env.BASE_URL3}/stokopname/batch?obat=${idKartuStok}&depo=10`;
