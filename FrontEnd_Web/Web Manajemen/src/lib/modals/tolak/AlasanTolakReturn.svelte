<script lang="ts">
	...
	export let isKonfirmTolakReturnOpen = false;
</script>
	...
	>
		<div class="my-auto {width} rounded-xl bg-white drop-shadow-lg" on:click|stopPropagation>
			<div class="flex items-center justify-between rounded-t-xl bg-[#6988DC] p-8">
				<div class="font-montserrat text-[26px] text-white">Alasan Tolak Return Barang</div>
				<button
			...
			<form class="mb-4 mt-6 px-8 pb-3">
				<div class="mt-2 flex flex-col gap-2">
					<TextArea id="alasan" label="Alasan Return Barang ini ditolak" placeholder="Alasan" />
				</div>
