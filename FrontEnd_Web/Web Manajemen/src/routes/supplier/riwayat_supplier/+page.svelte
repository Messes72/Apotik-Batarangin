		...
		>
			<div class="my-auto w-[1000px] rounded-xl bg-white drop-shadow-lg" on:click|stopPropagation>
				<div class="flex items-center justify-between rounded-t-xl bg-[#6988DC] p-8">
					<div class="font-montserrat text-[26px] text-white">Informasi Data Supplier</div>
					<button class="rounded-xl hover:bg-gray-100" on:click={() => (isModalDetailOpen = false)}>
						<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="none"
							><path
								fill="#fff"
								d="M12.8 38 10 35.2 21.2 24 10 12.8l2.8-2.8L24 21.2 35.2 10l2.8 2.8L26.8 24 38 35.2 35.2 38 24 26.8 12.8 38Z"
				...
				<form class="my-6 px-8 pb-3">
					<div class="mt-2 flex flex-col gap-2">
						<Detail label="Nama Supplier" value="Alexander Yofilio" />
						<Detail label="Alamat Supplier" value="jl.Sudriman No. 45, Surabaya" />
						<Detail label="Nomor Telepon Supplier" value="081234567890" />
						<Detail label="Catatan Supplier" value="Catatan Supplier" />
					</div>
				</form>
			</div>
		</div>
	{/if}
	<KonfirmDelete bind:isOpen={isModalKonfirmDeleteOpen} bind:isSuccess />
	<Hapus bind:isOpen={isSuccess} />
</div>
